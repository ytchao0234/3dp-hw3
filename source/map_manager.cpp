...
// Move from p0 to p1
// Return p
// If no collision, p = p1
// If collision, set p to be a point before collision
//
void MAP_MANAGER::computePositionBasedOnMeshMapMgr(
    const Vector3 &p0
    , const Vector3 &p1
    , Vector3 &p
    )
{
	...
	Vector3 pp = 0.5*(p1+p0); // middle point

    // v: obstacle indicator at new position p1
    // v0: obstacle indicate at the middle position pp
	float v = mMeshMapMgr->getGridCellValue(p1.x, p1.z);  
	float v0 = mMeshMapMgr->getGridCellValue(pp.x, pp.z);

	if (v < 0.5 && v0 < 0.5) return; // no collision for both of them
