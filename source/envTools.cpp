...
// Then return.
//
// (CASE 2) Otherwise, cast a ray downward at position cur_pos.
// If it hits the terrain surface, cur_pos is above the terrain surface.
// Get the intersection point. Set new_pos.y = intersection_point.y + offset.
// Then return.
//
bool clampToEnvironment(const Vector3& cur_pos, Real offset, Vector3& new_pos)
{
	...
	//
	// Cast a ray downward
	// Then repeat the same process for handing the ray that is cast upward.
	//
	// Set a ray that is cast downward, e.g., -Vector3::NEGATIVE_UNIT_Y;
