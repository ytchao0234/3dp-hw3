...
public:
    FilledRectangle_Bar(
		...
		, uint8 renderQueue = RENDER_QUEUE_8
		...
		float left, float top, float right, float bottom)
    {
		...
		//basicTool_logMessage("mCur_value:", mCur_value);
		//basicTool_logMessage("mMax_value:", mMax_value);
		//basicTool_logMessage("f:", f);
...
public:
	...
	BAR_2D(SceneManager *sceneMgr
		, const String &materialName
		, const SceneNode *sceneNode
		, const SceneNode *camNode
		, bool flg_use_identity = true
		, uint8 renderQueue = RENDER_QUEUE_8);
	void setSplit2Parts(bool flg) {
	...
	Real getCurrentValue() const { return mCur_value; }
