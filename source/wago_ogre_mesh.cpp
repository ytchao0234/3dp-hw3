		...
		for (i = 0; i < nx; i++) {
			...
			if ((b->at(i, j) > 0.5) && !b->gr(i, j+1, 0.5)) {
				...
				if (flg_start) {
					...
					//if (i>=nx-1) {
					v1 = Vector3(x+dx, 0, z+dz);
					v2 = Vector3(x+dx, h, z+dz);
					//} else {
					//	v1 = Vector3(x, 0, z+dz);
					//	v2 = Vector3(x, h, z+dz);
					//}
					...
					//
					//cout << "end start:" << i << "\t" << j << endl;
					//cout << "end start:" << v1.x << "\t" << v1.z << endl;
					flg_start = false;
					nf++;
				} else {
					//cout << "start:" << i << "\t" << j << endl;
					//cout << "start:" << x << "\t" << z+dz << endl;
					v0 = Vector3(x, 0, z+dz);
					v3 = Vector3(x, h, z+dz);
					flg_start = true;
				}
...
void SIMPLE_TERRAIN::scanMapForLocatingObjects()
{
	...
	for (int j = 0; j < nz; ++j)
	{
		for (int i = 0; i < nx; ++i) {
			...
			if (b > t) {
				//blue
				mStartingPosition = Vector3(x, 100, z);
			}
